...
class ControlTower
{
    ...
    public function applyForLanding($aircraft_name)
    {
        ...
        if ($runway_number) {
            echo sprintf("もう着陸許可は下りているわ、%s番滑走路を使って。\n", $runway_number);
        } else {
            $allow = false;
            foreach ($this->landing as $key => $runway) {
                if (!$runway) {
                    ...
                    $allow = true;
                    echo sprintf("%s こちら管制塔。着陸を許可する。%s番滑走路を使ってください。\n", $aircraft_name, $key);
                    return false;
                }
            }
            if (!$allow) {
                echo sprintf("%sへ、こちら管制塔。現在滑走路は埋まっているので少し待ってください。\n", $aircraft_name);
            }
        }
    }
}